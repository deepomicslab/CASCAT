...
def plot_pesudotime(emd, group_frac, connectivities, predict_labels, pesudotime, label_map, show=True, save_path=None,
    ...
    def get_centriods(emb, pesudotime, group_frac):
        ...
        center_size = pd.DataFrame(np.zeros([group_frac.shape[1], 1]), columns=['size'], index=group_frac.columns)
        for i in group_frac.columns:
            idx = np.where(predict_str_labels == i)[0]
            center_pos.loc[i, :] = np.mean(emb[idx, :], axis=0)
            center_pesudotime.loc[i, :] = np.mean(pesudotime.iloc[idx])
            center_size.loc[i, :] = len(idx)
        return center_pos, center_pesudotime, center_size

    fig, ax = plt.subplots(1, 1, figsize=(10, 10))
    reducer = umap.UMAP(n_components=2, random_state=5, n_jobs=1)
    emd_2pc = reducer.fit_transform(emd)
    centroids, center_pesudotime, center_size = get_centriods(emd_2pc, pesudotime, group_frac)
    ...
    sns.scatterplot(x=emd_2pc[:, 0], y=emd_2pc[:, 1], c=[sm.to_rgba(i) for i in pesudotime], s=15, ax=ax, legend=False)
    node_colors_mapped = [sm.to_rgba(center_pesudotime.loc[i, :]) for i in sorted(center_pesudotime.index)]
    nx.draw_networkx_nodes(graph, pos={i: centroids.loc[i, :] for i in centroids.index},
                           node_size=center_size.values * 10,
                           node_color=node_colors_mapped, ax=ax, alpha=0.8)
    nx.draw_networkx_edges(graph, pos={i: centroids.loc[i, :] for i in centroids.index}, arrows=True, ax=ax,
                           arrowstyle="-|>", connectionstyle="arc3,rad=-0.2", width=2, arrowsize=30)
