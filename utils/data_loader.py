...
def load_data_from_raw(args):
    ...
    if 'cluster' not in adata.obs.keys():
        gt_clusters = pd.Series(index=adata.obs_names, dtype=int)
    ...
    return gene_exp, labels, nclasses, adj_knn, dis
...
def load_labels_from_emb(path, nclasses, adata, num_neighbors=5, emb_key="metric_embedding",
                         label_key='metric_clusters', adj_key='knn_adj', is_undirected=True):
    emb = np.load(path)
    ...
    adjacency = np.zeros(shape=(adata.n_obs, adata.n_obs), dtype=np.float64)
    dis = distance_matrix(emb, emb)
    neighbors_idx = np.argsort(dis, axis=1)
    for i, n_idx in enumerate(neighbors_idx):
        n_idx = n_idx[n_idx != i][:num_neighbors]
        adjacency[i, n_idx] = 1
    ...
    if is_undirected:
        print('Use undirected cell-cell communication graph')
        adjacency = ((adjacency + adjacency.T) > 0).astype(int)
    adata.obsp[adj_key] = csr_matrix(adjacency)
    return adata
