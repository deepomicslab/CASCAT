...
class SpaceFlow(object):
    ...
    def __init__(self, adata=None, count_matrix=None, spatial_locs=None, sample_names=None, gene_names=None):
        ...
        if adata and isinstance(adata, anndata.AnnData):
            ...
            print(
                "Please input either an anndata.AnnData or both the count_matrix (count matrix of gene expression, 2D int numpy array of size (n_cells, n_genes)) and spatial_locs (spatial locations of cells (or spots) in 1D float numpy array of size (n_locations,)) to initiate SpaceFlow object.")
            exit(1)

    def plt_setting(self, fig_title_sz=30, font_sz=12, font_weight="bold", axes_title_sz=12, axes_label_sz=12,
                    xtick_sz=10, ytick_sz=10, legend_font_sz=10):
        """
    ...
    def train(self, spatial_regularization_strength=0.1, z_dim=50, lr=1e-3,
              epochs=1000, max_patience=50, min_stop=100, random_seed=42, gpu=0, regularization_acceleration=True,
              edge_subset_sz=1000000, trial=0):
        ...
        expr = adata_preprocessed.X.todense() if type(
            adata_preprocessed.X).__module__ != np.__name__ else adata_preprocessed.X
    ...
    def evaluation(self, embedding, labels):
        ...
        for clu_trial in range(5):
            ...
            ari_ls.append(ari)
            ami_ls.append(ami)
        return np.mean(ari_ls), np.mean(ami_ls), predict_labels
