...
class Experiment:
    ...
    def run_spaceflow(self):
        best_emb = self.sf.train(spatial_regularization_strength=0.1, z_dim=50, lr=1e-2, epochs=1000, max_patience=50,
                                 min_stop=100, random_seed=self.args.seed, gpu=0, regularization_acceleration=True,
                                 edge_subset_sz=1000000)
        self.data.obsm["X_emb"] = best_emb
        x_coord = self.data.obsm['spatial']
        ...
        # sc.pp.neighbors(self.data, n_neighbors=self.args.n_neighbors, random_state=self.args.seed)  # for fast running
        # self.data.obsp['connectivities'] = get_CMI_connectivities(self.data, self.args.CMI_dir, percent=self.percent)
        resolution, _ = run_leiden(self.data, self.nclasses, random_state=self.args.seed)
...
def parse_arguments():
    ...
    parser.add_argument('--dataname', type=str, default='OSCC1')
    ...
    parser.add_argument('--s', type=int, default=10, help='size of the scatter plot')
