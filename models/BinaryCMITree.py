...
from InformationMeasure.KDEMeasures import get_kde_cond_mutual_info
from InformationMeasure.Measures import get_conditional_mutual_information


class BinaryCMITree:

    def __init__(self, adata, root, predict_key, threshold=0.095, seed=0, save_dir=None, kde=True):
        self.seed = seed
    ...
    def cal_CMI_matrix(self, root):
        ...
        CMI_path = f'{self.save_dir}/trial{self.seed}/{str(root)}_CMI.csv'
        print(f"calculate CMI matrix based on root: {root} on {CMI_path}")
        if os.path.exists(CMI_path):
            CMIDF = pd.read_csv(f'{self.save_dir}/trial{str(self.seed)}/{str(root)}_CMI.csv', index_col=0, header=0)
            CMIDF.columns = CMIDF.columns.astype(str)
            CMIDF.index = CMIDF.index.astype(str)
        else:
            leaves = [n for n in self.tree.nodes if n != root]
            CMIDF = pd.DataFrame(np.zeros(shape=(len(leaves), len(leaves))), index=leaves, columns=leaves)
            combs = itertools.combinations(leaves, 2)
            filter_combs = [comb for comb in combs if comb[0] != comb[1]]
            for leaf_i, leaf_j in filter_combs:
                if self.kde:
                    CMI, self.samples = get_kde_cond_mutual_info(nodes=(leaf_i, leaf_j, root),
                                                                 samples=self.samples, express=self.tree.nodes)
                else:
                    expr_i, expr_j = self.tree.nodes[leaf_i]['express'], self.tree.nodes[leaf_j]['express']
                    CMI = get_conditional_mutual_information(valuesX=expr_i, valuesY=expr_j,
                                                             valuesZ=self.tree.nodes[root]['express'])
                print(f"leaf_i: {leaf_i}, leaf_j: {leaf_j}, CMI: {CMI + 3.2}")
                CMIDF.loc[leaf_i, leaf_j], CMIDF.loc[leaf_j, leaf_i] = CMI + 3.2, CMI + 3.2
            CMIDF.to_csv(CMI_path, index=True, header=True)
        return CMIDF
